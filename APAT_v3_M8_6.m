classdef APAT_v3_M8_6 < matlab.apps.AppBase
...
%Error: "Unrecognized method, property, or field 'DataTipTemplate' for class 'matlab.graphics.primitive.Surface'"
    ...
    methods (Access = private)
        ...
        function applyChoices(app, newSource)
            ...
            sp = app.Single_DropDown_cutValue; sp.Limits = [-Inf Inf]; sp.Step = step;
    ...
    methods (Access = private)
        ...
        function renderFull(app)
            ...
            if isempty(g.surf) || ~isvalid(g.surf) || ~isequal(g.size, size(C))
                ...
                g.surf = surface(ax, X, Y, Z, C, 'EdgeColor', 'none', 'ContextMenu', app.PlotMenu); g.size = size(C);
