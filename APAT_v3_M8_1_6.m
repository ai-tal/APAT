classdef APAT_v3_M8_1_6 < matlab.apps.AppBase
...
% Error: "Index exceeds array bounds"
    ...
    methods (Access = private)
        ...
        function update(app, scope, arg)
            ...
            if scope == "export", app.exportMain(arg); return; end
            if scope == "browse"                                         % UI entry of "source": ask for the file
            [f, p] = uigetfile(app.FileFilter, 'Select an antenna pattern file', app.startDir());
            if isequal(f, 0), return; end
            arg = fullfile(p, f); scope = "source";
            elseif scope == "reset"                                      % UI entry of "params": restore defaults
                app.applyParams(app.DefaultParams); scope = "params";
            end
            is = @(varargin) any(scope == string(varargin));             % bound only now: an anonymous function captures scope BY VALUE (M8.1 fix)
    ...
    methods (Static)
        function ok = selfTest()
            ...
            K = APAT_v3_M8_1_6.Const; A6 = APAT_v3_M8_1_6.Axes6;
